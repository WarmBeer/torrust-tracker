...
pub async fn update_one_torrent_in_parallel<T: TRepositorySync + Send + Sync + 'static>(runtime: &tokio::runtime::Runtime, samples: usize) -> (Duration, Duration) {
    let args = Args::parse();
    let mut results: Vec<Duration> = Vec::with_capacity(samples);
...
pub async fn add_multiple_torrents_in_parallel<T: TRepositorySync + Send + Sync + 'static>(runtime: &tokio::runtime::Runtime, samples: usize) -> (Duration, Duration) {
    let args = Args::parse();
    let mut results: Vec<Duration> = Vec::with_capacity(samples);
...
pub async fn update_multiple_torrents_in_parallel<T: TRepositorySync + Send + Sync + 'static>(runtime: &tokio::runtime::Runtime, samples: usize) -> (Duration, Duration) {
    let args = Args::parse();
    let mut results: Vec<Duration> = Vec::with_capacity(samples);
