...
use torrust_tracker::tracker::torrent::repository::{TRepositoryAsync};
use crate::args::Args;
use crate::benches::utils::{DEFAULT_PEER, generate_unique_info_hashes, get_average_and_adjusted_average_from_results};
...
pub async fn async_update_one_torrent_in_parallel<T: TRepositoryAsync + Send + Sync + 'static>(runtime: &tokio::runtime::Runtime, samples: usize) -> (Duration, Duration) {
    let args = Args::parse();
    let mut results: Vec<Duration> = Vec::with_capacity(samples);
...
pub async fn async_add_multiple_torrents_in_parallel<T: TRepositoryAsync + Send + Sync + 'static>(runtime: &tokio::runtime::Runtime, samples: usize) -> (Duration, Duration) {
    let args = Args::parse();
    let mut results: Vec<Duration> = Vec::with_capacity(samples);
...
pub async fn async_update_multiple_torrents_in_parallel<T: TRepositoryAsync + Send + Sync + 'static>(runtime: &tokio::runtime::Runtime, samples: usize) -> (Duration, Duration) {
    let args = Args::parse();
    let mut results: Vec<Duration> = Vec::with_capacity(samples);
